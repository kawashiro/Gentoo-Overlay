...
SLOT="0"
KEYWORDS="~x86 ~amd64"
IUSE="pulseaudio avahi cddb cdda dvd aac flac lirc mad matroska modplug musepack mpeg
...
DEPEND="media-libs/aalib
		...
		|| ( media-video/acestream-ffmpeg[pulseaudio=,aac=,modplug=,ieee1394=,speex=,theora,v4l=,vaapi,vorbis,alsa]
			 media-video/ffmpeg:0.10[pulseaudio=,aac=,modplug=,ieee1394=,speex=,theora,v4l=,vorbis,alsa] )
		sys-apps/dbus
		media-video/dirac
...
src_install(){
	...
	dosym "liblua.so" "/usr/$(get_libdir)/liblua5.2.so.0"
	dosym "liba52.so" "/usr/$(get_libdir)/liba52-0.7.4.so"
	dosym "libdvbpsi.so" "/usr/$(get_libdir)/libdvbpsi.so.8"
	dosym "libBasicUsageEnvironment.so" "/usr/$(get_libdir)/libBasicUsageEnvironment.so.0"
	dosym "libgroupsock.so" "/usr/$(get_libdir)/libgroupsock.so.1"
	dosym "libliveMedia.so" "/usr/$(get_libdir)/libliveMedia.so.23"
	dosym "libUsageEnvironment.so" "/usr/$(get_libdir)/libUsageEnvironment.so.1"

	use pulseaudio || rm "${D}/usr/lib/acestreamplayer/plugins/access/libpulsesrc_plugin.so" "${D}/usr/lib/acestreamplayer/plugins/services_discovery/libpulselist_plugin.so" "${D}/usr/lib/acestreamplayer/plugins/audio_output/libpulse_plugin.so"
